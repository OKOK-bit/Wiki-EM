...
for i in range(len(leak_mode_list)):
    if leak_mode_list[i] == "sign":
        ...
        single_infer = True
    elif leak_mode_list[i] == "fedem":
        fedem_eps = float(leak_mode_list[i + 1])  # 例如 fedem-1
...
def leakage_dataset(images, labels, net):
    if not args.dataset.startswith("wikitext"):
        ...
        if fedem_eps > 0:
            if args.dataset.startswith("wikitext") or args.dataset.startswith("cola"):
                # 对 NLP token 做离散整数扰动（±fedem_eps）
                fedem_eps_int = int(fedem_eps)  # 确保为整数
                noise = torch.randint_like(images, low=-fedem_eps_int, high=fedem_eps_int + 1)
                images = torch.clamp(images + noise, 0, voc_size - 1)
            else:
                # 对图像做连续高斯扰动（输入为 0~1 的像素）
                noise = torch.randn_like(images) * fedem_eps
